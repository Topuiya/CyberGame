...
@interface PubgVC () <UITableViewDelegate, UITableViewDataSource>

@property (weak, nonatomic) IBOutlet UITableView *tableView;

@end

@implementation PubgVC

NSString *PubgID = @"Pubg";
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:NSStringFromClass([RankTableViewCell class]) bundle:nil] forCellReuseIdentifier:PubgID];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return _PubgModel.list.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    RankTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:PubgID forIndexPath:indexPath];
    RankListModel *model = _PubgModel.list[indexPath.row];
    ...
    if (indexPath.row == 0) {
        ...
        for (int i = 0; i < _PubgModel.list.count;i++) {
